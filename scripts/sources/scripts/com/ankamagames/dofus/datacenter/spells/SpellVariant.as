package com.ankamagames.dofus.datacenter.spells
{
   ...
   public class SpellVariant implements IDataCenter
   {
      ...
      public function toString() : String
      {
         ...
         var id0:int = 0;
         var id1:int = 0;
         if(Boolean(this.spells.length) && Boolean(this.spells[0]))
         {
            name0 = String(this.spells[0].name);
         }
         if(this.spells.length > 1 && Boolean(this.spells[1]))
         {
            name1 = String(this.spells[1].name);
         }
         if(Boolean(this.spellIds.length) && Boolean(this.spellIds[0]))
         {
            id0 = int(this.spellIds[0]);
         }
         if(this.spellIds.length > 1 && Boolean(this.spellIds[1]))
         {
            id1 = int(this.spellIds[1]);
         }
