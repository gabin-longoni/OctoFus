package com.ankamagames.dofus.datacenter.items.criterion
{
   ...
   public class AchievementPioneerItemCriterion extends ItemCriterion implements IDataCenter
   {
      ...
      override public function get isRespected() : Boolean
      {
         ...
         {
            pioneerRank = int((questFrame.finishedCharacterAchievementByIds[achievementId] as AchievementAchieved).achievedPioneerRank);
         }
         else
         {
            pioneerRank = int(questFrame.achievementPioneerRanks[achievementId]);
            if(pioneerRank == 0)
            {
               pioneerRank = 1;
            }
