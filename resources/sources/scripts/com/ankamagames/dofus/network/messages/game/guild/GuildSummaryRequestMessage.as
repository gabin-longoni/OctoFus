package com.ankamagames.dofus.network.messages.game.guild
{
   ...
   public class GuildSummaryRequestMessage extends PaginationRequestAbstractMessage implements INetworkMessage
   {
      ...
      public function deserializeAs_GuildSummaryRequestMessage(input:ICustomDataInput) : void
      {
         ...
         var _criterionFilterLen:uint = uint(input.readUnsignedShort());
         ...
         var _languagesFilterLen:uint = uint(input.readUnsignedShort());
         ...
         var _recruitmentTypeFilterLen:uint = uint(input.readUnsignedShort());
         for(var _i6:uint = 0; _i6 < _recruitmentTypeFilterLen; _i6++)
         {
            _val6 = uint(input.readByte());
            if(_val6 < 0)
            {
      ...
      {
         var _val:uint = uint(input.readByte());
         if(_val < 0)
         {
