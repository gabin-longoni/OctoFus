package com.ankamagames.dofus.network.types.game.mount
{
   ...
   public class MountClientData implements INetworkType
   {
      ...
      public function deserializeAs_MountClientData(input:ICustomDataInput) : void
      {
         ...
         var _ancestorLen:uint = uint(input.readUnsignedShort());
         ...
         var _behaviorsLen:uint = uint(input.readUnsignedShort());
         ...
         var _effectListLen:uint = uint(input.readUnsignedShort());
      ...
      {
         var _val:uint = uint(input.readInt());
         if(_val < 0)
         {
