package com.ankamagames.dofus.datacenter.items
{
   ...
   public class Item implements IPostInit, IDataCenter
   {
      ...
      private const BOSS_BONUS:Number = 5;
      ...
      public static function getItemById(id:uint, returnDefaultItemIfNull:Boolean = true) : Item
      {
         ...
         if(Boolean(item) || !returnDefaultItemIfNull)
         {
            return item;
         }
      ...
      public function get recipes() : Array
      {
         ...
         if(!this._recipes)
         {
            numRecipes = int(this.recipeIds.length);
      ...
      {
         var bossBonus:Number = this.isBossResource(itemWrapper) ? this.BOSS_BONUS : 1;
         var craftBonus:Number = isCraft ? this.CRAFT_BONUS : 1;
      ...
      private function isBossResource(itemWrapper:ItemWrapper) : Boolean
      {
         ...
         for each(monsterId in itemWrapper.dropMonsterIds)
         {
            ...
            if(Boolean(monster) && !monster.isBoss)
            {
               return false;
            }
         }
      ...
      {
         return Boolean(this.evolutiveEffectIds) && this.evolutiveEffectIds.length > 0;
      }
